...
if [ ! "$(docker ps -q -f name=${docker_runnable_name})" ]; then
    ...
    docker run --name ${docker_runnable_name} -it -p 5432:5432 -e POSTGRES_PASSWORD=password -v /home/igor/IdeaProjects/Forum-API/postgres/volumes/init.sql:/docker-entrypoint-initdb.d/init.sql postgres
fi
